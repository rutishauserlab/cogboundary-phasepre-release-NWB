...
function [] = B01_raster_psth_sceneRecog_SAligned_BSep_NWB(nwbData, timestampsOfCell, channelid, cellNr, brainAreaOfCell, bin_width_raster, bin_width_fr, plot_flag)
...
stimCategory = recognition_table.vectordata.get('boundary_type').data.load();  % NB, SB, HB
...
figure_dir = [pwd, '/NWB/exported/Figures/CBID', num2str(subjectID), '/SAligned_sceneRecog/'];
if ~exist(figure_dir)
    mkdir(figure_dir)
end

figure_name = ['Raster_psth_SAligned', cellLabelStr, '_sceneRecog.png'];
